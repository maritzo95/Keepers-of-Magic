...
*/
	...
	void Start ()
	{
		...
		//Settingthe health for each of the units
		/*
...
*/
	...
	{//change so one doesnt spawn on other objects.
		if ((cc.selected && firstPlayerTurn && cc.tag.Equals ("Player 1")) || (cc.selected && !firstPlayerTurn && cc.tag.Equals ("Player 2"))) {
			for (int x = 0; x < sizeX; x++) {
				for (int y = 0; y < sizeY; y++) {
					if (Distance ((int)selectedUnit.transform.position.x, (int)selectedUnit.transform.position.y, x, y) <= cc.maxMoveDistance && Distance ((int)selectedUnit.transform.position.x, (int)selectedUnit.transform.position.y, x, y) > 0 && tileOpen (clickTiles [x, y]) && this.moveMode) {
						//Debug.Log("X =" + x + "Y=" + y);
					...
					if (Distance ((int)selectedUnit.transform.position.x, (int)selectedUnit.transform.position.y, x, y) <= cc.attackRange && Distance ((int)selectedUnit.transform.position.x, (int)selectedUnit.transform.position.y, x, y) > 0 && checkEnemy (clickTiles [x, y]) && !this.moveMode) {
						//Debug.Log("X =" + x + "Y=" + y);
